...
export default function FridayPage() {
  ...
  return <main className="friday-page"><section className="friday-hero"><p>One agent · private workspaces</p><h1>Friday<br />Access.</h1><span>Một Friday duy nhất trên server; mỗi user là một workspace, session và quota riêng. Friday Admin trên server là nơi duy nhất duyệt quyền.</span></section><section className="friday-grid"><article className="friday-card"><p className="friday-label">01 · Access</p><h2>{approved ? "Friday đã sẵn sàng" : access?.status === "pending" ? "Đang chờ Friday Admin duyệt" : "Xin quyền dùng Friday"}</h2><p>{approved ? "Workspace riêng của bạn đã được cấp." : "Friday Admin trên server sẽ duyệt, đặt quota và tạo workspace/session riêng cho bạn."}</p>{approved ? <div className="friday-quota"><strong>{Math.ceil((access?.window_used_seconds ?? 0) / 60)}/{access?.hourly_limit_minutes} phút</strong><span>đã dùng trong 60 phút</span></div> : <button className="friday-action" onClick={requestAccess} disabled={loading || access?.status === "pending"}>{access?.status === "pending" ? "Đã gửi yêu cầu" : "Xin quyền sử dụng"}</button>}{access?.admin_note ? <small>{access.admin_note}</small> : null}</article><article className="friday-card"><p className="friday-label">02 · API keys</p><h2>Keys của bạn</h2><p>Key chỉ hiện một lần và có thể thu hồi bất kỳ lúc nào.</p>{approved ? <><form className="friday-key-form" onSubmit={createKey}><input value={keyLabel} onChange={(event) => setKeyLabel(event.target.value)} maxLength={80} required /><button disabled={loading}>Tạo API key</button></form>{apiKey ? <div className="friday-secret"><strong>Sao chép ngay — key này sẽ không hiện lại.</strong><code>{apiKey}</code></div> : null}<div className="friday-key-list">{access?.keys.map((key) => <div key={key.id}><span><b>{key.label}</b><small>{key.prefix}… · {key.status}</small></span>{key.status === "active" ? <button onClick={() => revokeKey(key.id)}>Revoke</button> : null}</div>)}</div></> : <div className="friday-locked">Cần được Friday Admin duyệt trước khi tạo API key.</div>}</article></section>{error ? <p className="friday-error">{error}</p> : null}</main>;
}
